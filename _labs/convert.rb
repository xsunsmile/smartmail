...
#文字コード判定
print "\n元ファイル" +  src + "の文字コードは "
print CODES[NKF.guess(contents)]
print "のようです。\n\n"

#バイナリとASCIIと判別できないものは処理しません
if NKF.guess(contents)>=4 && NKF.guess(contents)<=6
    print "この形式は変換できません!\n\n"
    exit	#処理中断
end
...
#元がEUCでなければ
#EUCファイルの作成
unless NKF.guess(contents)==2
    CS::conv(original,"euc",contents)
end

#元がJISでなければ
#JISファイルの作成
unless NKF.guess(contents)==1
    CS::conv(original,"jis",contents)
end

#元がUTFでなければ
#UTFファイルの作成
unless NKF.guess(contents)==7
    CS::conv(original,"utf",contents)
end
